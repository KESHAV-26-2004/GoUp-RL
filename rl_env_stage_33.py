...
FPS = 60
PLAYER_VEL = 6
TOTAL_ROWS = 26          # 25 pattern rows + 1 win row
...
class GoUpEnv:
    ...
    def _generate_objects(self):
        ...
        # ============================================================
        # PLATFORM GENERATION : ZIG-ZAG TOWER (25 + 1 win)
        # ============================================================
        ...
        tower_x = random.randint(MIN_X, MAX_X)
        tower_x = (tower_x // 96) * 96

        for row in range(TOTAL_ROWS):

            # Every 6th row (after every 5 straight) offset left/right
            # Skip offset on the final win row
            if row > 0 and row % 6 == 5 and row != TOTAL_ROWS - 1:
                direction = random.choice([-1, 1])
                offset_blocks = random.choice([1, 2])
                new_x = tower_x + direction * offset_blocks * 96

                # If out of bounds, flip direction
                if new_x < MIN_X or new_x > MAX_X - 192:
                    direction *= -1
                    new_x = tower_x + direction * offset_blocks * 96

                tower_x = max(MIN_X, min(MAX_X - 192, new_x))
                tower_x = (tower_x // 96) * 96
    ...
    def reset(self, seed=None):
        ...
        # Win platform = last generated row (26th block)
        ...
        self.max_blocks = TOTAL_ROWS
        self.block_gap = 192
        self.target_height = self.a_yaxis[TOTAL_ROWS - 1]
        ...
        self.reward_config = {
            ...
            # platform progression (scales to 25+ rows)
            "platform_base_reward": 10.0,
            "platform_step_reward": 5.0,
            ...
            # falling penalty
            "fall_penalty_per_step": 0.05,
            "fall_penalty_threshold": 500.0,

            # invalid action
            "invalid_jump_penalty": 1.0,
        }
    ...
    def _handle_vertical_collision(player, objects, dy):
        ...
        for obj in objects:
            if not pygame.sprite.collide_mask(player, obj):
                continue

            # Side walls must never act as floor/ceiling.
            if getattr(obj, "rl_static_role", None) in ("wall_left", "wall_right"):
                continue

            # Require actual horizontal overlap with the object.
            horizontal_overlap = (
            ...
            if dy > 0:
                ...
                # Jumping: only hit the underside.
                if player.rect.top <= obj.rect.bottom:
    ...
    def step(self, action):
        ...
        # ============================================================
        # BEST HEIGHT TRACKING & FALLING PENALTY
        # ============================================================

        # Update best height (lower y = higher up)
        if self.player.rect.y < self.best_y:
            self.best_y = float(self.player.rect.y)

        # Penalize falling too far below best height
        fall_threshold = self.reward_config.get("fall_penalty_threshold", 100.0)
        fall_penalty_per_step = self.reward_config.get("fall_penalty_per_step", 0.05)

        if self.player.rect.y > self.best_y + fall_threshold:
            # print("[falling penalty]")
            reward -= fall_penalty_per_step
            reward_breakdown["falling"] = -fall_penalty_per_step
        ...
        if landed_platform_id is not None:
            if landed_platform_id not in self.visited_platform_ids:
                self.visited_platform_ids.add(landed_platform_id)
                ...
                platform_reward = (
                    self.reward_config["platform_base_reward"]
                    + row_index * self.reward_config["platform_step_reward"]
                )
                ...
                if row_index >= TOTAL_ROWS - 2:
                    reward += 50
                    reward_breakdown["goal_bonus"] = 50
        ...
        if landed_platform_id == self.final_platform_id:
            ...
            reward_breakdown["terminal"] = self.reward_config["win_reward"]

        # ============================================================
        # FALL DEATH
        # ============================================================

        if self.player.rect.y > self.y_out + 200 and not done:
            done = True
            print("death: fell out of world")
            terminal_reason = "death"
            reward -= self.reward_config["death_penalty"]
            reward_breakdown["terminal"] = -self.reward_config["death_penalty"]

        # ============================================================
        # TIME LIMIT
        # ============================================================

        if self.steps >= 1800 and not done:
            done = True
        ...
        reward_breakdown["time_penalty"] = -self.reward_config["time_penalty"]
