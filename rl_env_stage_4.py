...
class GoUpEnv:
    ...
    def _generate_objects(self):
        ...
        # ============================================================
        # PLATFORM GENERATION : STAGE 4 TOWER (DIAGONAL STAIRCASE)
        # ============================================================
        ...
        # Fixed X positions
        LEFT_X   = 280   # start extreme left
        MID_L_X  = 420   # mid-left
        MID_R_X  = 600   # mid-right
        RIGHT_X  = 1080  # far right
        CENTER_X = 550   # large + win platforms

        # Decide start side (randomly left or right)
        start_left = random.choice([True, False])

        TOTAL_ROWS = 7

        for row_idx in range(TOTAL_ROWS):
            y = self.a_yaxis[row_idx]

            if row_idx == 0:
                # Row 1: START (small, extreme side)
                base_x = LEFT_X if start_left else RIGHT_X
                num_blocks = 2
                ...
                # Row 3: SMALL (first step of diagonal)
                base_x = MID_L_X if start_left else MID_R_X
                num_blocks = 2

            elif row_idx == 3:
                # Row 4: SMALL (second step of diagonal, opposite mid)
                base_x = MID_R_X if start_left else MID_L_X
                num_blocks = 2

            elif row_idx == 4:
                # Row 5: SMALL (third step, far side)
                base_x = RIGHT_X if start_left else LEFT_X
                num_blocks = 2
    ...
    def reset(self, seed=None):
        ...
        self.reward_config = {
            ...
            "fall_speed_threshold":20.0,
            "fall_speed_penalty_scale":2.0,

            # invalid action
            "invalid_jump_penalty": 1,
        }
    ...
    def step(self, action):
        ...
        # Optional: prevent camera from going below 0 (if you don't want to show below floor)
        ...
        if self.steps >= 800 and not done:
